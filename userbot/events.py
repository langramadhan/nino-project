...
from userbot import bot, BOTLOG_CHATID, LOGSPAMMER, CMD_HANDLER, CMD_LIST, SUDO_HANDLER, SUDO_USERS
    ...
    **args,
    ...
    if pattern is not None:
        ...
        if (
            ...
            nino_reg = nino_reg = re.compile(pattern)
        else:
            nino_ = "\\" + CMD_HANDLER
            sudo_ = "\\" + SUDO_HANDLER
            nino_reg = re.compile(nino_ + pattern)
            ...
            if command is not None:
                ...
                cmd1 = (
                    (nino_ + pattern).replace("$", "").replace("\\", "").replace("^", "")
                )
                cmd2 = (
                    (sudo_ + pattern)
                    .replace("$", "")
        ...
        if bot:
            if not disable_edited:
                bot.add_event_handler(
                    func, events.MessageEdited(**args, outgoing=True, pattern=nino_reg)
                )
            bot.add_event_handler(
                func, events.NewMessage(**args, outgoing=True, pattern=nino_reg)
            )
        if bot:
            if allow_sudo:
                if not disable_edited:
