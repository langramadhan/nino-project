...
from userbot.utils import edit_delete, edit_or_reply, nino_cmd
...
@nino_cmd(pattern="addsudo(?:\s|$)([\s\S]*)")
async def add(event):
    suu = event.text[9:]
...
@nino_cmd(pattern="delsudo(?:\s|$)([\s\S]*)")
async def _(event):
    if event.sender_id in SUDO_USERS:
        return
    suu = event.text[8:]
