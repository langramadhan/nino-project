...
async def get_user_from_event(
    event, secondgroup=None, nogroup=False, noedits=False
):
    if nogroup is False:
        if secondgroup:
    ...
    try:
        ...
        if event.reply_to_msg_id:
            ...
            if previous_message.sender_id is None:
                if not noedits:
                    await edit_delete("**ERROR: Dia adalah anonymous admin!**", 60
                    )
                return None, None
        ...
        if not args:
            if not noedits:
                await edit_delete("**Mohon Reply Pesan atau Berikan User ID/Username pengguna!**",
                    60,
                )
            return None, None
    except Exception as e:
